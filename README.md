# Console-Checkout-Calculator
want to know the cost? here it is
README with purpose, setup, operation, and known limitations.
written by Ivan Zheng


what this calculator is meant to do is to simpify the math on a purchase, starting with the price than taxes, coupon discounts, then final result.
it works by putting in a price. than it gets subtracted by coupons, and then divded by taxes
limitations im aware of is that its stuck on one option, discount larger than subtotal is possible but it goes to negative, it also cant do advanaced math.

most diffcult in my opinion is to branch out the code to go into if statements, i thought it was needing to just stick with week 2 but it became clear that i needed more to make it work.
